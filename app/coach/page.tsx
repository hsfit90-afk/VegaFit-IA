...
export default function Coach() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] md:h-full w-full max-w-4xl mx-auto md:p-6 pb-24 md:pb-6 relative animate-fade-in">
      <header className="p-4 border-b border-border bg-background/80 backdrop-blur-xl sticky top-0 z-10 md:rounded-t-[24px] md:border md:border-b-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-primary flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-primary animate-pulse"></span> Online (Groq AI)</p>
          </div>
        </div>
      </header>
      ...
      <div className="p-4 bg-background/90 backdrop-blur-xl border-t border-border md:rounded-b-[24px] md:border relative z-20">
        {messages.length <= 2 && (
          <div className="flex overflow-x-auto pb-4 gap-2 no-scrollbar">
            {SUGGESTIONS.map((sug, i) => (
              <button 
                ...
                className="whitespace-nowrap px-4 py-2 bg-surface hover:bg-surface-light border border-border rounded-full text-xs text-gray-300 transition-colors"
              >
                {sug}
              </button>
            ))}
        ...
        <div className="relative">
          <input 
            ...
            className="w-full bg-surface border border-border rounded-2xl py-4 pl-4 pr-12 text-white focus:border-accent focus:ring-1 focus:ring-accent outline-none transition-all"
            disabled={isLoading}
          />
          <button 
