...
import { ArrowLeft, Dumbbell, Save, Plus, Trash2, Search, X } from 'lucide-react';
...
export default function TrainerManualWorkoutCreator() {
  ...
  useEffect(() => {
    async function loadData() {
      if (!clientId) return;
      const { data } = await supabase.from('profiles').select('name').eq('id', clientId).single();
  ...
  const handleSave = async () => {
    if (sessions.some(s => s.exercises.length === 0)) {
      alert('Por favor, adicione pelo menos um exercício a cada sessão de treino.');
      return;
    }
    ...
    const { error } = await supabase.from('workout_plans').insert({
      id: planId,
      user_id: clientId, // Salvando PARA o aluno
      name: planName,
      split: split,
      sessions: sessions
    });
    ...
    if (error) {
      ...
      alert('Treino salvo e enviado para o aluno com sucesso!');
    ...
    >
      <motion.div variants={itemVariants} className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-foreground-muted text-sm">Montando ficha para: {clientProfile?.name || 'Carregando...'}</p>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <motion.div variants={itemVariants} className="md:col-span-2 space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Defina o nome e a divisão do treino do seu aluno.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  placeholder="Ex: Treino de Hipertrofia (Mês 1)"
                />
              </div>
              <div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>Adicione os exercícios para cada dia da semana.</CardDescription>
              </div>
                      ...
                      {session.exercises.map((ex) => (
                        <div key={ex.id} className="flex flex-col gap-2 p-3 bg-background rounded-lg border border-border">
                          ...
                          <div className="mt-1">
                             <input 
                                type="text" 
                                placeholder="Dicas para execução (Opcional)"
                                value={ex.tips || ''}
                                onChange={(e) => updateExercise(index, ex.id, 'tips', e.target.value)}
                                className="w-full bg-surface border border-border rounded px-2 py-1.5 text-sm text-white focus:border-primary outline-none"
                              />
                          </div>
                        </div>
                      ))}
        ...
        <motion.div variants={itemVariants} className="space-y-6">
          ...
          >
            <Save className="w-5 h-5 mr-2" /> {isSaving ? 'SALVANDO...' : 'ENVIAR TREINO'}
          </Button>
        </motion.div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in" onClick={() => setIsExerciseModalOpen(false)}>
          <Card className="w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl relative border-primary/30" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
            <CardHeader className="pb-4 border-b border-border">
              <div className="flex justify-between items-center mb-4">
                <CardTitle>Buscar na Biblioteca</CardTitle>
